...
config:
  ...
  relation:
    -
      table: t_product             # *** Must be set and have a primary key. The primary key will generate the id of /index/type/id in es, if has multi primary key, id where append with "-"
