...
config:
 ...
      increment-column: ["id"]     # *** Must be set. Indicates that it is used for data increment operations, using increment `id` or `update_time`
      ...
      justAdd: true                # Whether the data has been increasing and will not be updated, the default is false, true will not check whether the data exists to decide whether to add or update will be added directly
