...
config:
  ...
  count: 2000                      # 可以不设定, 默认是 10000, 一次往 es 操作数据的批量条数
 ...
      increment-column: ["id"]     # *** 必须设定. 表示用来做数据增量操作时用, 一般使用自增 id 或 updateTime(更新时间戳)
      ...
      key-column: ["id"]           # 主键列, 如果不设置将会自动从表中查询, 如果是多列主键会用 "-" 拼接, 这个配置可以设置成只使用具体的列(如果设置, 请只使用一列, 并保证这个列是唯一的)
      ...
      just-add: true               # 数据是否一直在增加并不会更新, 默认是 false, 为 true 将不检查数据是否存在来决定是添加还是更新将直接添加
